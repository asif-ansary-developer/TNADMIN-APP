...
public class StatusBar {
  ...
  private boolean shouldSetStatusBarColor(boolean hasOptOut) {
    ...
    if (deviceApi < Build.VERSION_CODES.R) {
      // device below Android 15 - can always set status bar
      canSetStatusBar = true;
    } else if (deviceApi == Build.VERSION_CODES.R) {
  ...
  private boolean isEdgeToEdgeOptOutEnabled(Window window) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
      // Fallback: manually resolve the attribute using reflection or hardcoded int
      int resId = window.getContext().getResources().getIdentifier(
        "windowOptOutEdgeToEdgeEnforcement", "attr", "android");
      if (resId != 0) {
        TypedValue value = new TypedValue();
        window.getContext().getTheme().resolveAttribute(resId, value, true);
        return value.data != 0;
      }
    }
    return false;
  ...
  private int getStatusBarHeight() {
    ...
    WindowInsetsCompat insetsCompat = ViewCompat.getRootWindowInsets(activity.getWindow().getDecorView());
    if (insetsCompat != null) {
      return insetsCompat.getInsets(WindowInsetsCompat.Type.statusBars()).top;
    }
